...
class InitialSolution:
    ...
    @staticmethod
    def generate_initial_solution_elite_threshold(data, alpha=0.7, max_time=60):
        """
        Deterministic 'value-based RCL' initializer extracted from GRASP's construction.
        - No randomness, no local search.
        - Fully adaptive: recomputes marginal gain each step based on current time and scanned books.
        - Value-based RCL threshold: include candidates with score >= s_min + alpha*(s_max - s_min).
        - Deterministic tie-break: prefer smaller signup_days, then higher books_per_day, then higher score.
        """
        ...
        while time.time() - start_time < max_time and libs and curr_time < data.num_days:
            candidates = []
            for lib in libs:
                # feasibility: must finish signup before horizon ends
                ...
                # top unscanned books by score, limited by capacity
                available = sorted(
                ...
                score = sum(data.scores[b] for b in available)
                if score > 0:
                    candidates.append((score, lib, available))

            if not candidates:
                break

            s_min = min(s for s, _, _ in candidates)
            s_max = max(s for s, _, _ in candidates)
            cutoff = s_min + alpha * (s_max - s_min)

            rcl = [(s, lib, books) for (s, lib, books) in candidates if s >= cutoff]

            # deterministic pick from RCL:
            # 1) smallest signup_days, 2) highest books_per_day, 3) highest score
            s, best_lib, best_books = sorted(
                rcl,
                key=lambda x: (x[1].signup_days, -x[1].books_per_day, -x[0]),
            )[0]
        ...
        # remaining libs become unsigned
        for lib in libs:
            unsigned_libs.append(lib.id)
    ...
    @staticmethod
    def generate_initial_solution_ordered_list(data):
        Library._id_counter = 0
    ...
    @staticmethod
    def generate_initial_solution_static_greedy(data):
        Library._id_counter = 0
        lib_info = []
    ...
    @staticmethod
    def generate_initial_solution_weighted_efficiency(data, alpha=1, beta=0.1, max_time=60):
        start_time = time.time()
    ...
    @staticmethod
    def generate_initial_solution_penalty_greedy(data, time_limit=60):
        start_time = time.time()
        best_score = 0
        best_alpha = 1.0
        best_beta = 0.1
    ...
    @staticmethod
    def generate_initial_solution_adaptive_heap(data):
    ...
    def generate_initial_solution(data):
        ...
        best_alpha, best_beta, best_score, weighted_solution = (
            InitialSolution.generate_initial_solution_penalty_greedy(data, time_limit=60)
        )
        ...
        generation_methods = [
            ...
            (
                InitialSolution.generate_initial_solution_adaptive_heap,
                {},
                "Greedy"
            ),
            ...
            (
                InitialSolution.generate_initial_solution_ordered_list,
                {},
                "Sorted"
            ),
        ]
